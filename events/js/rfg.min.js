(function ($) {
        ...
        rfg.ajax = {};
        rfg.ajax.checkInValue = 0;
        rfg.ajax.buildFlickrCalls = function () {
            var i = 0,
                setCoutner = rfg.options.flickrSets.length;
            for (i = 0; i < rfg.options.flickrSets.length; i++) {
                var singleSet = rfg.options.flickrSets[i];
                var infoApiCall = "http://api.flickr.com/services/rest/?format=json&method=flickr.photosets.getInfo&photoset_id=" +
                    singleSet.setId + "&per_page=18&page=1&api_key=" + singleSet.apiKey + "&jsoncallback=?";
                rfg.ajax.makeFlickrCalls(infoApiCall, singleSet);
            }
        };
        rfg.ajax.makeFlickrCalls = function (infoApiCall, singleSet) {
            $.getJSON(infoApiCall, function (data) {
                var category = data.photoset.title._content;
                var apiCall = "http://api.flickr.com/services/rest/?format=json&method=flickr.photosets.getPhotos&photoset_id=" +
                    singleSet.setId + "&per_page=18&page=1&api_key=" + singleSet.apiKey + "&jsoncallback=?";
                $.getJSON(apiCall, function (data) {
                    ...
                    $.each(data.photoset.photo, function (i, photo) {
                        ...
                        var image = $('<img/>').attr('src', img_src).load(function () {
                            var link = $('<a></a>').attr('href', img_src).attr('rel', 'lightbox[on]').append(image);
                            if (rfg.options.flickrOptions.useTitle === true) {
                                link.attr('title', photo.title);
                            }
                            var unit = $('<div></div>').attr('data-category', category).append(link);
                            rfg.images.imageArray.push(unit);
                            imageCount++;
                            if (imageCount === data.photoset.photo.length) {
                                rfg.ajax.checkInValue++;
                                if (rfg.ajax.checkInValue === rfg.options.flickrSets.length) {
        ...
        rfg.lightbox.addOverlay = function () {
            var units = rfg.el.find('.rfg-images > div').css({
                'cursor': 'pointer'
            }).append('<span></span>');
        ...
        rfg.images.show = function () {
            rfg.el.find('.rfg-images > div').css('opacity', '0').css('visibility', 'visible').each(function () {
                $(this).animate({
                    'opacity': '1'
                }, {
